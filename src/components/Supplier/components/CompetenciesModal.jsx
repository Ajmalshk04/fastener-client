...
const CompetenciesModal = ({ onCompetenciesChange, initialCompetencies = [] }) => {
  const [selectedCompetencies, setSelectedCompetencies] = useState(initialCompetencies);
      ...
      <DialogContent className="sm:max-w-[700px]">
        ...
        <ScrollArea className="h-[60vh] pr-4">
          <div className="grid grid-cols-3 gap-6">
            {Object.entries(competenciesData).map(([category, competencies]) => (
              <div key={category} className="mb-4">
                <h3 className="font-semibold mb-2">{category}</h3>
                {competencies.map((competency) => (
                  <div key={competency} className="flex items-center space-x-2 mb-1">
                    <Checkbox
                      id={competency}
                      checked={selectedCompetencies.includes(competency)}
                      onCheckedChange={() => handleCompetencyChange(competency)}
                    />
